...
export const HomePage = ({ onSelectPrompt, purchasedPrompts }) => {
  ...
  useEffect(() => {
    ...
    if (activeAiType)     params.append("aiType", activeAiType);
    params.append("sort", `${sortType},desc`);
  ...
  const handleAiTypeChange   = (value) => { setActiveAiType(value);   setPage(0); };
  const handleSortChange     = (sort)  => { setSortType(sort);        setPage(0); };
  ...
  return (
      <div className="min-h-screen" style={{ background: "#0b0b12", position: "relative" }}>
        ...
        <section className="px-4 mb-3" style={{ position: "relative", zIndex: 1 }}>
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
              {CATEGORIES.map(cat => (
                  <button key={cat.label} onClick={() => handleCategoryChange(cat.value)} className="shrink-0 px-4 py-1.5 rounded-full text-sm transition-all"
                          style={activeCategory === cat.value
                              ? { background: "var(--primary)", color: "#fff" }
                              : { background: "var(--muted)", color: "#ffffff", border: "1px solid var(--border-sm)" }}>
                    {cat.label}
                  </button>
              ))}
            </div>
        ...
        <section className="px-4 mb-6" style={{ position: "relative", zIndex: 1 }}>
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center gap-2 overflow-x-auto pb-2" style={{ scrollbarWidth: "none" }}>
              {AI_TYPES.map(type => (
                  <button key={type.label} onClick={() => handleAiTypeChange(type.value)} className="shrink-0 px-4 py-1.5 rounded-full text-sm transition-all"
                          style={activeAiType === type.value
                              ? { background: "var(--brand-violet-light)", color: "#fff" }
                              : { background: "var(--muted)", color: "#ffffff", border: "1px solid var(--border-sm)" }}>
                    {type.label}
                  </button>
              ))}
          ...
          <div className="max-w-7xl mx-auto">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                <h2 className="font-semibold" style={{ color: "var(--foreground)" }}>프롬프트 목록</h2>
                <span className="text-sm" style={{ color: "var(--muted-foreground)" }}>{totalElements}개</span>
              </div>
              <div className="flex items-center gap-1">
                <button onClick={() => handleSortChange("createdAt")} className="px-3 py-1 rounded-full text-xs transition-all"
                        style={sortType === "createdAt"
                            ? { background: "var(--primary)", color: "#fff" }
                            : { background: "var(--muted)", color: "#ffffff", border: "1px solid var(--border-sm)" }}>
                  최신순
                </button>
                <button onClick={() => handleSortChange("rating")} className="px-3 py-1 rounded-full text-xs transition-all"
                        style={sortType === "rating"
                            ? { background: "var(--primary)", color: "#fff" }
                            : { background: "var(--muted)", color: "#ffffff", border: "1px solid var(--border-sm)" }}>
                  평점순
                </button>
            ...
            {totalPages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-10">
                  <button
                      ...
                      disabled={page === 0}
                      className="flex items-center gap-1 px-3 py-1.5 rounded-lg text-sm transition-all disabled:opacity-30"
                      style={{ background: "var(--muted)", color: "#ffffff", border: "1px solid var(--border-sm)" }}>
