...
import { Star, Download, Edit2, Crown, Code2, CheckCircle2, TrendingUp } from "lucide-react";
...
export function ProfilePage({ isPremium, onUpgradePremium }) {
          ...
          <div className="rounded-2xl p-6 mb-6 relative overflow-hidden" style={{ background: "var(--card)", border: "1px solid var(--border-md)" }}>
            ...
            <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center relative">
              <div className="relative">
                <div className="w-20 h-20 rounded-2xl flex items-center justify-center text-3xl font-bold text-white" style={{ background: "var(--gradient-profile)" }}>윤</div>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex flex-wrap items-center gap-2 mb-1">
                  <h1 className="font-bold" style={{ color: "var(--foreground)", fontSize: "1.25rem" }}>윤혜민</h1>
                  {isPremium && (
                      <span className="flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium" style={{ background: "var(--gold-bg)", color: "var(--brand-gold)", border: "1px solid var(--gold-border-md)" }}>
                    <Crown size={10} /> 프리미엄
                  </span>
                  )}
                ...
                <p className="text-sm mb-2" style={{ color: "var(--muted-foreground)" }}>dev.hyemin@example.com</p>
                <p className="text-sm" style={{ color: "var(--secondary-foreground)" }}>스타일 통합 담당 · Spring Boot / React 개발자</p>
              </div>
              {!isPremium && (
                  <button onClick={onUpgradePremium} className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-opacity hover:opacity-90"
                          style={{ background: "var(--gold-bg)", color: "var(--brand-gold)", border: "1px solid var(--gold-border-md)" }}>
                    <Crown size={13} /> 프리미엄 업그레이드
                  </button>
              )}
            </div>
          ...
          {activeTab === "overview" && (
              <div className="space-y-4">
                <div className="rounded-xl p-5" style={{ background: "var(--card)", border: "1px solid var(--border-sm)" }}>
                  <h3 className="text-sm font-medium mb-3" style={{ color: "var(--foreground)" }}>구독 상태</h3>
                  {isPremium ? (
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-full flex items-center justify-center" style={{ background: "var(--gold-bg)" }}>
                          <Crown size={18} style={{ color: "var(--brand-gold)" }} />
                        </div>
                        <div>
                          <p className="font-medium" style={{ color: "var(--foreground)" }}>프리미엄 구독 중</p>
                          <p className="text-xs" style={{ color: "var(--muted-foreground)" }}>다음 결제일 2025-06-20 · 월 29,000원</p>
                        </div>
                        <button className="ml-auto text-xs px-3 py-1.5 rounded-lg" style={{ background: "var(--muted)", color: "var(--muted-foreground)", border: "1px solid var(--border-sm)" }}>구독 관리</button>
                      </div>
                  ) : (
                      <div className="flex items-center justify-between">
                        <p className="text-sm" style={{ color: "var(--muted-foreground)" }}>무료 회원 · 샘플 및 가이드 열람 가능</p>
                        <button onClick={onUpgradePremium} className="text-xs px-3 py-1.5 rounded-lg font-medium" style={{ background: "var(--brand-gold)", color: "var(--brand-gold-foreground)" }}>업그레이드</button>
                      </div>
                  )}
                </div>
                <div className="rounded-xl p-5" style={{ background: "var(--card)", border: "1px solid var(--border-sm)" }}>
