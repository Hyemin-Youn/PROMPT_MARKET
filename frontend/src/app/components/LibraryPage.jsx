...
export function LibraryPage({ purchasedPrompts, onLogout, onSelectPrompt }) {
                ...
                <div className="space-y-3 mb-8">
                  {PURCHASED.map((item, i) => (
                      <button key={item.id} onClick={() => setActivePrompt(i)} className="w-full flex items-center gap-4 p-4 rounded-xl text-left transition-all"
                              style={{ background: activePrompt === i ? "var(--primary-bg-xs)" : "var(--card)", border: `1px solid ${activePrompt === i ? "var(--border-2xl)" : "var(--border-sm)"}` }}>
