...
export const AuthModal = ({ mode, onClose, onSuccess, onSwitchMode }) => {
  ...
  const Field = ({ icon: Icon, label, name, type = "text", placeholder, right }) => (
    <div>
      ...
      <div className="flex items-center gap-2 px-3 py-2.5 rounded-lg" style={{ background: "var(--muted)", border: `1px solid ${errors[name] ? "var(--destructive)" : "var(--border-md)"}` }}>
      ...
      {errors[name] && <p className="text-xs mt-1 flex items-center gap-1" style={{ color: "var(--destructive)" }}><AlertCircle size={11} />{errors[name]}</p>}
    </div>
  );
