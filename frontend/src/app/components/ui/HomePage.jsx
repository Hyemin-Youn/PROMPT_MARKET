...
import { User, ShoppingBag, Heart, Settings, Copy, CheckCheck, ChevronRight, Crown, Download } from "lucide-react";
...
export const LibraryPage = ({ purchasedPrompts, onLogout, onSelectPrompt }) => {
  ...
  return (
      <div className="min-h-screen flex" style={{ background: "var(--background)" }}>
        ...
        <aside className="w-48 shrink-0 hidden md:flex flex-col gap-1 p-4 pt-8" style={{ background: "var(--sidebar)", borderRight: "1px solid var(--border-xs)" }}>
          <div className="flex items-center gap-2 px-2 py-3 mb-4">
            ...
            <div className="min-w-0">
              <p className="text-sm font-medium truncate" style={{ color: "var(--foreground)" }}>윤혜민</p>
              {purchasedPrompts.length > 0 && (
                  <span className="text-xs flex items-center gap-0.5" style={{ color: "var(--brand-gold)" }}>
                <Crown size={10} /> 프리미엄
              </span>
              )}
            </div>
        ...
        <div className="flex-1 min-w-0 pb-20 md:pb-0">
                ...
                <div className="space-y-3 mb-8">
                  {PURCHASED.map((item, i) => (
                      <button
                          ...
                          style={{
                            // 💡 배경을 고정하여 보라색 그라데이션 배경 위에서도 형태를 또렷하게 보호합니다.
                            background: "var(--card)",
                            // 💡 선택되었을 때만 강한 보라색 보더 테두리가 나타납니다.
                            border: `1px solid ${activePrompt === i ? "var(--border-2xl)" : "var(--border-sm)"}`,
                            // 💡 선택 시 은은한 광채 그림자를 주어 고급스러운 다크 모드 감성을 더했습니다.
                            boxShadow: activePrompt === i ? "0 0 15px var(--primary-bg-md)" : "none"
                          }}
                      >
          ...
          {activeNav === "profile"   && <ProfilePage isPremium={purchasedPrompts.length > 0} onUpgradePremium={() => {}} />}
          {activeNav === "favorites" && <FavoritesPage onSelectPrompt={onSelectPrompt} />}
          {activeNav === "settings"  && <SettingsPage isPremium={purchasedPrompts.length > 0} onLogout={onLogout} onUpgradePremium={() => {}} />}
        </div>
      </div>
  );
}